    ...
    p101_env_set_fault_injector(env, NULL, NULL);
    {
        ...
        if(native_pid == 0)
        {
            ...
            if(p101_error_has_error(native_err))
            {
                ...
                {
                    fprintf(stderr, "native smoke produced an undeclared platform failure: p101_posix_memalign: %s\n", p101_error_get_message(native_err));
                    native_passed = false;
                }
        ...
        if(native_pid > 0)
        {
            ...
            {
                fprintf(stderr, "native smoke fixture unavailable: p101_posix_memalign\n");
            }
